...
socket.on("mensajes", async (mensajes) => {
  ...
  const rowTemplate = await fetchRow.text();
  const mensajesDenormalizados = normalizr.denormalize(mensajes.result, chatSchema, mensajes.entities);
  ...
  const porcentajeComprimido = (100 - Math.round((pesoNormalizado * 100)/pesoDenormalizado));

  console.log(`Porcentaje Comprimido: ${porcentajeComprimido}%`);

  document.querySelector(".tituloChat").innerText = `Chat (Comprimido ${porcentajeComprimido}%)`;
...
const enviarMensaje = async (e) => {
  ...
  socket.emit('mensajePost', {author: { id, nombre, apellido, edad, alias, avatar }, text});
}
