  ...
  if (cartId) {
    ...
    for (let p of cartProducts) {
      const indice = cartProductsAux.indexOf(
        cartProductsAux.find(({ id }) => id === p.id)
      );
      indice !== -1
    ...
    if (cartProductsAux.length) {
      const cartRows = cartProductsAux
        .map(({ quantity, id, name, price }) => {
          return `<tr>
            <th scope="row">${quantity}</th>
            <td>${name}</td>
            <td>$${price}</td>
            <td style="cursor: pointer" data-id="${id}" class="btnEliminarProducto">X</td>
            </tr>`;
        })
        .join("\n");
...
const renderEditarProducto = async (e) => {
  ...
  const producto = await productoRef.json();
  contenedorProductos.innerHTML = `<form class="row justify-content-center bg-light p-4" id="formEditarProducto" data-id="${producto.id}">
    <h4>Actualizar Producto</h4>
    <div class="mb-3 col-auto">
      <label for="nombre" class="form-label">Nombre</label>
      <input type="text" class="form-control" name="nombre" value="${producto.name}" required>
    </div>
    <div class="mb-3 col-auto">
      <label for="precio" class="form-label">Precio</label>
      <input type="number" class="form-control" name="precio" value="${producto.price}" required>
    </div>
    <div class="mb-3 col-auto">
    ...
    <div class="mb-3 col-7">
        ...
        <input type="text" class="form-control" name="imagen" value="${producto.photoUrl}" required>
      </div>
