  ...
  if (cartId) {
    ...
    for (let p of cartProducts) {
      const indice = cartProductsAux.indexOf(
        cartProductsAux.find(product => { return '_id' in p ? product._id === p._id : product.id === p.id})
      );
      indice !== -1
      ...
      const cartRows = cartProductsAux
        .map((product) => {
          return `<tr>
            ...
            <td>${product.name}</td>
            <td>$${product.price}</td>
            <td style="cursor: pointer" data-id="${product.id || product._id}" class="btnEliminarProducto">X</td>
            </tr>`;
        })
        .join("\n");
